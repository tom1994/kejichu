...
<div class="top-content">
    ...
    <div class="top-img">
        <div id="KinSlideshow" style="visibility:hidden;">
            <!--取得该分类下加入轮播图的文章-->
            {foreach GetList(null,53) as $key=>$article}
            {if $article->Metas->lunbo==1}
            <a href="{$article.Url}" target="_blank"><img src="{$article.Metas.pic}" alt="{$article.Title}"/></a>
            {/if}
            {/foreach}
            <!--取得该分类下加入轮播图的文章-->
            {foreach GetList(null,54) as $key=>$article}
            {if $article->Metas->lunbo==1}
            <a href="{$article.Url}" target="_blank"><img src="{$article.Metas.pic}" alt="{$article.Title}"/></a>
            {/if}
            {/foreach}
            <!--取得该分类下加入轮播图的文章-->
            {foreach GetList(null,55) as $key=>$article}
            {if $article->Metas->lunbo==1}
            <a href="{$article.Url}" target="_blank"><img src="{$article.Metas.pic}" alt="{$article.Title}"/></a>
            {/if}
            {/foreach}
    ...
    <div id="div-content" class="top-art">
        <div id="firstPage" class="cms-up show">
            ...
            <a class="gjzl-more" href="{$categorys[53].Url}">更多</a>
            <ul>
                {foreach GetList(10, 53) as $key=>$article}
        ...
        <div id="secondPage" class="cms-up hide">
            <h2 class="gjzl-title">内部研究</h2>
            <a class="gjzl-more" href="{$categorys[54].Url}">更多</a>
            <ul>
                {foreach GetList(10, 54) as $key=>$article}
                <li><a href="{$article.Url}">{$article.Title}</a></li>
                {/foreach}
            </ul>
        </div>
        <div id="thirdPage" class="cms-up hide">
            <h2 class="gjzl-title">内部研究</h2>
            <a class="gjzl-more" href="{$categorys[55].Url}">更多</a>
            <ul>
                {foreach GetList(10, 55) as $key=>$article}
                <li><a href="{$article.Url}">{$article.Title}</a></li>
                {/foreach}
            </ul>
        </div>
...
<div class="cms-content">
    ...
    <div class="cms-left">
        <div class="cms-left-title">
            <h2>研究观点</h2>
            <span><a href="{$categorys[56].Url}"><img src="{$host}zb_users/theme/{$theme}/image/focus.png"></a></span>
        </div>
        <div class="tu-f">
            <!--取得该分类下第一个置顶文章-->
            {$topArray = GetList(1, 56, null, null, null, null, array("only_ontop"  => true));}
        ...
        <div class="cms-lr-list">
            <ul>
                {foreach GetList(4,56) as $key=>$article}
                <li><a href="{$article.Url}">{$article.Title}</a></li>
                {/foreach}
            </ul>
        </div>
    ...
    <div class="cms-right">
        <div class="cms-right-title">
            <h2>科研成果</h2>
            <span><a href="{$categorys[57].Url}"><img src="{$host}zb_users/theme/{$theme}/image/focus.png"></a></span>
        </div>
        <div class="tu-f">
            <!--取得该分类下第一个置顶文章-->
            {$topArray = GetList(1, 57, null, null, null, null, array("only_ontop"  => true));}
            {foreach $topArray as $top}
            <a href="{$top.Url}"><img src="{$top.Metas.pic}" alt="{$top.Title}">
                <h3>{$top.Title}</h3>
            </a>
        ...
        <div class="cms-lr-list">
            <ul>
                {foreach GetList(4,57) as $key=>$article}
                <li><a href="{$article.Url}">{$article.Title}</a></li>
                {/foreach}
            </ul>
        </div>
